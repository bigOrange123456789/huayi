...
class DynamicLoading{
    ...
    start2(){//用于渲染的遮挡剔除
		...
		function setInterval0(){
            ...
            if(Object.keys(scope.lists).length<1)//还没有加载完可见性列表
                return
            if(true){//始终进行遮挡剔除判断 //if(prePoint2!==point0){//如果视点位置变化就进行遮挡剔除判断
                if(window.meshes)
    ...
    updateSource(min,step,max,path,preload) {//用于加载和预加载
        ...
        if(!preload){//加载
            for(var j=0;j<lists.length;j++){
                ...
                setTimeout(()=>{
                    for(var i=Math.floor(list.length/2);i<Math.floor(2*list.length/2);i++)
                        scope.loading(list[i],false)
                },200)
                // setTimeout(()=>{
    ...
    updateSource2(min,step,max,path,preload) {//用于渲染
        ...
        for(var i in lists)
            if(lists[i].length>0)
                for(var j=0;j<lists[i].length&&j<20;j++)//for(var j in lists[i])
...
window.test00=function(){
    var frustum = new Frustum();
    frustum.setFromProjectionMatrix( new Matrix4().multiplyMatrices( window.c.projectionMatrix, window.c.matrixWorldInverse ) );
    ...
    console.log(planes)
    {
        ...
        for(var ii=0;ii<6;ii++){//for(var ii=0;ii<planes.length;ii++){
            var direction = new Vector3(
                -1*window.c.matrixWorld.elements[8],-1*window.c.matrixWorld.elements[9],-1*window.c.matrixWorld.elements[10]
            )
            var x=window.c.position.x
            var y=window.c.position.y
            var z=window.c.position.z
    ...
    if(false){//
        ...
        for(var ii=5;ii<6;ii++){//for(var ii=0;ii<planes.length;ii++){
            var direction = new Vector3(
                -0.1*window.c.matrixWorld.elements[8]
                ,-0.1*window.c.matrixWorld.elements[9]
                ,-0.1*window.c.matrixWorld.elements[10]
            )
            var normal=planes[ii].normal
            console.log(
